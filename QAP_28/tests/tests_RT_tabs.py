...
# check negative chek aut by phone 
...
#check negative email auth
def test_010_negative_by_email(selenium):
    form = AuthForm(selenium)

    # email enter
    form.username.send_keys('123@123.123')
    form.password.send_keys('any_password')
    sleep(5)
    form.btn_click()
...
# check time code and form for phone ane code enter 
...
# chek user agreement open
def test_agreement(selenium):
    form = AuthForm(selenium)

    original_window = form.driver.current_window_handle
    # click "Пользовательским соглашением" в подвале страницы
    form.agree.click()
    sleep(5)
...
# check link translation vk auth
...
#check link translation OK auth
...
#check link translation mail.ru auth
...
#check link translation google auth
...
#check link translation yandex auth
